...
from constants import CHASE_HIGH_VALUE
from util.util import iterate_file_names


def construct_learning_sample(ticker, study_period):
    raw_data = prepare_raw_data(ticker)
    study_samples = []
    empty_sample = [0,0,0,0,0,0,0,0]
    for i in range(len(raw_data)-1):
        if raw_data[i+1][CLOSE]/raw_data[i][CLOSE] > CHASE_HIGH_VALUE:
            study_sample = []
            if i < study_period:
...
def combine_study_samples(key_ticker, study_period, checking_condition):
    total_study_samples = []
    for ticker in iterate_file_names():
        if checking_condition(key_ticker, ticker):
            total_study_samples += construct_learning_sample(ticker, study_period)
